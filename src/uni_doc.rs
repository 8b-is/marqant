...
impl UniversalDocument {
    ...
    pub fn classify_genre(text: &str) -> (DocumentGenre, f32) {
        ...
        let date_patterns = [
            ...
            "19", "20" // century markers
        ];
        
        let date_count = date_patterns.iter()
    ...
    pub fn extract_emotional(text: &str) -> EmotionalContext {
        ...
        EmotionalContext {
            ...
            intensity: primary.1,
            emotion_blend: scores.into_iter()
                .filter(|(_, s)| *s > 0.0)
...
pub fn publisher_to_universal(pub_text: &str) -> UniversalDocument {
    ...
    let cleaned = pub_text
        ...
        .filter(|line| {
            ...
            !line.contains("") && 
            line.chars().filter(|c| c.is_ascii_graphic() || c.is_whitespace()).count() > line.len() / 2
        })
        .collect::<Vec<_>>()
        .join("\n");
