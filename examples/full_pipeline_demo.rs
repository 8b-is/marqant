...
use marqant::{
    ...
    utl_enforced::{Eng, Jpn, RawText, RawToUtl, Spa, Translate, UtlDoc, UtlToHuman},
    utl_phonetics::{decode_compact, encode_compact},
};
...
fn main() {
    ...
    let documents = vec![
        ...
        (
            "poem.pub",
            "The stars remind me of your eyes, shining in the darkness",
        ),
    ...
    println!(
        "📚 Processing {} documents through UTL pipeline...\n",
        documents.len()
    );
    ...
    for (filename, content) in &documents {
        ...
        let symbols: Vec<&str> = utl_doc.tokens.iter().map(|s| s.as_str()).collect();
        let packets = encode_compact(&symbols);
        println!(
            "  🎵 Phonetic: {} packets ({} bytes)",
            packets.len(),
            packets.len() * 2
        );
        ...
        let wave_sample: Vec<String> = memory
            .wave_pattern
            .iter()
            .take(8)
            .map(|&v| {
    ...
    let total_text_bytes: usize = documents.iter().map(|(_, text)| text.len()).sum();
    ...
    println!(
        "  Compression: {}%",
        100 - (total_packet_bytes * 100 / total_text_bytes)
    );
