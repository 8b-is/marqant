...
use marqant::{
    utl_phonetics::{encode_compact, decode_compact, example_i_love_you},
...
fn main() {
    ...
    for (i, mem) in similar_memories.iter().enumerate() {
        ...
        let preview: String = phones.iter()
            .take(4)
            .map(|p| p.ph)
            .collect::<Vec<_>>()
            .join(" ");
        ...
        println!("   {}. {} (similarity: {:.1}%)", i + 1, preview, similarity * 100.0);
    }
    ...
    println!("  \"I love you\" vs \"You love me\": {:.1}%", love1.similarity(&love2) * 100.0);
    println!("  \"I love you\" vs \"I hate you\": {:.1}%", love1.similarity(&hate) * 100.0);
    ...
    let text_bytes: usize = thoughts.iter()
        .map(|(name, _)| name.len())
        .sum();
    ...
    println!("  Compression: ~{}%", 100 - (packet_bytes * 100 / (text_bytes * 3)));
